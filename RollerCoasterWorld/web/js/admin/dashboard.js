...
const StatManager = {
  ...
  bindEvents() {
    ...
    btns.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (period === "custom") {
          ...
          document.getElementById("range-error")?.classList.add("d-none");
    ...
    document.getElementById("btn-apply-range").addEventListener("click", () => {
      ...
      if (from > to) {
        if (errorEl) {
          errorEl.textContent = "La fecha inicial no puede ser posterior a la final";
          errorEl.classList.remove("d-none");
        }
        return;
      }
  ...
  initFlatpickr() {
    ...
    this.fpFrom = flatpickr("#range-from", {
      ...config,
      onChange: (selectedDates, dateStr) => {
        if (this.fpTo) this.fpTo.set("minDate", dateStr);
      },
    });

    this.fpTo = flatpickr("#range-to", {
      ...config,
      onChange: (selectedDates, dateStr) => {
        if (this.fpFrom) this.fpFrom.set("maxDate", dateStr);
      },
    });
  },
  ...
  async loadSummary() {
    try {
      ...
      if (data.success && data.data) {
        ...
        document.getElementById("kpi-coasters").innerText =
          s.total_coasters ?? "--";
        document.getElementById("kpi-parks").innerText = s.total_parks ?? "--";
        document.getElementById("kpi-reviews").innerText =
          s.total_reviews ?? "--";
        document.getElementById("kpi-photos").innerText =
          s.total_photos ?? "--";
  ...
  async loadGrowthCharts() {
    ...
    const reviewsData = await this.fetchGrowthData(
      "reviews",
      this.currentPeriod,
    );
    this.renderBarChart(
      "chart-growth-reviews",
      reviewsData,
      "Reseñas",
      "#17a2b8",
    );

    const forumData = await this.fetchGrowthData(
      "forum_posts",
      this.currentPeriod,
    );
  ...
  async fetchDistData(type) {
    try {
      const res = await fetch(
        `${window.DASHBOARD_API}?action=getDistribution&type=${type}`,
      );
      const data = await res.json();
      return data.success ? data.data : [];
    } catch (e) {
  ...
  renderBarChart(id, data, label, color) {
    ...
    this.charts[id] = new Chart(document.getElementById(id), {
      ...
      data: {
        labels: data.map((i) => i.label),
        datasets: [
          { label, data: data.map((i) => i.count), backgroundColor: color },
        ],
      },
      options: this.getChartOptions(),
    });
