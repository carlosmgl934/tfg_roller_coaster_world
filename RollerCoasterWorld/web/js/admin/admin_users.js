...
function _renderUsersTable(users) {
  ...
  users.forEach((user) => {
    ...
    const meta = [
      loc
        ? `<i class="fa-solid fa-location-dot me-1 text-success opacity-75"></i>${_escUser(loc)}`
        : null,
      dateStr
        ? `<i class="fa-regular fa-calendar me-1 opacity-50"></i>${dateStr}`
        : null,
      user.email
        ? `<i class="fa-solid fa-envelope me-1 opacity-50"></i>${_escUser(user.email)}`
        : null,
    ]
      .filter(Boolean)
      .join('<span class="mx-2 opacity-25">&bull;</span>');
                ...
                <div class="d-flex align-items-center gap-3 w-100">
                    ...
                    <div class="flex-grow-1 min-w-0">
                        <div class="d-flex align-items-center gap-2 flex-nowrap min-w-0">
                            <span class="fw-semibold text-white text-truncate" style="font-size:.95rem;">${_escUser(user.username)}</span>
                            <div class="flex-shrink-0">${roleBadge}</div>
                            <span class="text-muted font-monospace flex-shrink-0 d-none d-sm-inline" style="font-size:.7rem;">Nº ${String(user.id).padStart(6, "0")}</span>
                        </div>
                        <div class="text-muted text-truncate mt-1" style="font-size:.78rem; width: 100%;">
                            ${meta || "—"}
                        </div>
                    </div>
                    <div class="d-flex gap-1 gap-sm-2 flex-shrink-0">
                        <button class="btn btn-sm rounded-0 admin-action-btn px-2 px-sm-3" title="Editar"
                            onclick='openEditModal(${userJson})'>
                            <i class="fa-solid fa-pen-to-square"></i> <span class="d-none d-md-inline">Editar</span>
                        </button>
                        <button class="btn btn-sm rounded-0 admin-action-btn admin-action-del px-2 px-sm-3" title="Eliminar"
...
window.loadUsers = function () {
  ...
  $.ajax({
    url:
      `${window.BASE_URL}/api/php/admin/gestion_users.php?action=list` +
      `&page=${_usersPage}` +
      ...
      if (res.success) {
        ...
        $("#users-count").text(
          n === 1
            ...
            : `${n.toLocaleString()} usuarios encontrados`,
        );
      } else {
...
window.openEditModal = function (user) {
  ...
  // Nombre y métadatos
  document.getElementById("edit-user-display-name").textContent = user.full_name
  ...
  const joined = user.created_at
    ...
    : "";
  document.getElementById("edit-user-meta").textContent = [loc, joined]
    .filter(Boolean)
    .join(" · ");
...
$(document).ready(function () {
    ...
    $.ajax({
      ...
      success(res) {
        ...
        if (res.success) {
          ...
          _showUserInlineError(
            "#edit-user-error",
            res.error || "Error al eliminar foto",
          );
          $("#edit-user-messages").removeClass("d-none");
        }
      },
      error() {
        ...
        _showUserInlineError(
          "#edit-user-error",
          "Error de conexión al eliminar foto.",
        );
        $("#edit-user-messages").removeClass("d-none");
      },
      complete() {
  ...
  $("#admin-avatar-input").on("change", async function (e) {
    ...
    try {
      ...
      if (!compressedBlob)
        throw new Error(
          "No se pudo procesar la imagen (formato no soportado o corrupta).",
        );
      ...
      $.ajax({
        ...
        success(res) {
          if (res.success) {
            ...
            _showUserInlineError(
              "#edit-user-error",
              res.error || "Error al actualizar foto en base de datos",
            );
            $("#edit-user-messages").removeClass("d-none");
          }
          $btn.prop("disabled", false).html(originalHtml);
        ...
        error() {
          _showUserInlineError(
            "#edit-user-error",
            "Error de conexión al actualizar foto.",
          );
          $("#edit-user-messages").removeClass("d-none");
          $btn.prop("disabled", false).html(originalHtml);
        },
      ...
      if (
        ...
        errorMsg =
          "No se pudo procesar la imagen. Intenta con un archivo JPG o PNG válido.";
      }
  ...
  $("#confirm-delete-user").on("click", function () {
    ...
    $.ajax({
      ...
      error() {
        _showUserAlert("Error de conexión.");
      },
      complete() {
        $btn
          .prop("disabled", false)
          .html('<i class="fa-solid fa-trash me-1"></i>Eliminar usuario');
      },
    });
  });
