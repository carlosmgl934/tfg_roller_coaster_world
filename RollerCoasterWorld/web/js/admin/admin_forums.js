...
═══════════════════════════════════════════════════════════════ */
  document.addEventListener("DOMContentLoaded", () => {
    modalDetail = new bootstrap.Modal(
      document.getElementById("modal-forum-detail"),
    );
    modalDelForum = new bootstrap.Modal(
      document.getElementById("modal-delete-forum"),
    );
    modalDelMsg = new bootstrap.Modal(
      document.getElementById("modal-delete-message"),
    );
    modalUnban = new bootstrap.Modal(document.getElementById("modal-unban"));
    modalRemCollab = new bootstrap.Modal(
      document.getElementById("modal-remove-collab"),
    );
    modalBanOwner = new bootstrap.Modal(
      document.getElementById("modal-ban-owner"),
    );

    // Filtros y búsqueda
    document
      .getElementById("btn-forums-filtrar")
      .addEventListener("click", loadForums);
    document
      .getElementById("btn-forums-borrar")
    ...
    // Toggle ocultos
    document
      .getElementById("filter-show-hidden")
    ...
    document
      .getElementById("confirm-delete-forum")
      .addEventListener("click", confirmDeleteForum);
    ...
    document
      ...
      .addEventListener("click", confirmUnban);

    // Confirmar quitar colaborador
    document
      .getElementById("confirm-remove-collab")
      .addEventListener("click", confirmRemoveCollab);

    // Confirmar banear al creador (elimina el foro)
    document
      .getElementById("confirm-ban-owner")
    ...
    document
      .getElementById("btn-delete-forum-from-detail")
      .addEventListener("click", () => {
        ...
        <div class="list-group-item list-group-item-action rounded-0 py-3 px-3" data-forum-id="${f.id}">
            <div class="d-flex align-items-start justify-content-between gap-3 flex-wrap">
                <div class="flex-grow-1 min-w-0">
                    <div class="d-flex align-items-center gap-2 flex-nowrap mb-1 min-w-0">
                        <div class="flex-shrink-0">${privBadge}</div>
                        <span class="fw-bold text-white text-truncate">${escHtml(f.title)}</span>
                    </div>
                    <p class="text-muted small mb-1 text-truncate" style="width: 100%; max-width: none;">${escHtml(f.forum_subject)}</p>
...
═══════════════════════════════════════════════════════════════ */
  async function fetchMessages(forumId) {
    ...
    try {
      const resp = await fetch(
        `${API_FORUMS}?action=get_messages&forum_id=${forumId}&limit=100`,
        { credentials: "same-origin" },
      );
      const json = await resp.json();
      allMessages = json.messages || [];
      document.getElementById("tab-badge-msgs").textContent =
        allMessages.length;
      renderMessages();
    } catch {
      document.getElementById("messages-list").innerHTML =
  ...
  function renderMessages() {
    ...
    const msgs = showOnlyHidden
      ? allMessages.filter((m) => m.is_hidden)
      : allMessages;
    ...
    list.innerHTML = msgs
      .map((m) => {
        ...
        if (m.attachment_url) {
          ...
          const isImage = /\.(jpe?g|png|gif|webp|avif|svg)(\?|$)/i.test(
            m.attachment_url,
          );
          if (isImage) {
            attachmentHtml = `
                ...
                <div class="d-flex justify-content-between align-items-start gap-2 flex-wrap">
                    ...
                    <div class="d-flex gap-1 flex-shrink-0">
                        ...
                        <button class="btn btn-sm admin-action-btn admin-action-del rounded-0" title="Eliminar"
                                onclick="openDeleteMessage(${m.id}, '${escHtml(
                                  m.content || "",
                                )
                                  .substring(0, 80)
                                  .replace(/'/g, "\\'")}')">
  ...
  function openDeleteMessage(msgId, preview) {
    ...
    document.getElementById("delete-msg-preview").textContent =
  ...
  async function confirmDeleteMessage() {
    ...
    try {
      ...
      if (f) {
        f.msg_count = Math.max(0, +f.msg_count - 1);
        updateStats();
        renderForumList();
      }
    } catch (e) {
...
═══════════════════════════════════════════════════════════════ */
  async function fetchCollaborators(forumId) {
    ...
    try {
      const resp = await fetch(
        `${API_FORUMS}?action=get_collaborators&forum_id=${forumId}`,
        {
          credentials: "same-origin",
        },
      );
  ...
  async function confirmRemoveCollab() {
    ...
    try {
      ...
      if (f) {
        f.collab_count = Math.max(0, +f.collab_count - 1);
        updateStats();
        renderForumList();
      }
    } catch (e) {
...
═══════════════════════════════════════════════════════════════ */
  async function fetchBanned(forumId) {
    ...
    try {
      const resp = await fetch(
        `${API_FORUMS}?action=get_banned&forum_id=${forumId}`,
        {
          credentials: "same-origin",
        },
      );
  ...
  async function fetchParticipants(forumId) {
    ...
    try {
      const resp = await fetch(
        `${API_FORUMS}?action=get_participants&forum_id=${forumId}`,
        { credentials: "same-origin" },
      );
      const json = await resp.json();
    ...
    list.innerHTML = participants
      .map((p) => {
        const last = p.last_message_at
          ? new Date(p.last_message_at).toLocaleDateString("es-ES", {
              day: "2-digit",
              month: "short",
            })
          : "—";
        const isOwner =
          currentForumAuthorId && +p.user_id === +currentForumAuthorId;
  ...
  async function openBanUser(forumId, userId, username) {
    ...
    if (isOwner) {
      ...
      document.getElementById("ban-owner-forum-title").textContent =
        currentForumTitle;
      document.getElementById("delete-forum-title").textContent =
        currentForumTitle;
      modalBanOwner.show();
      return;
    }
  ...
  async function confirmUnban() {
    ...
    try {
      ...
      if (f) {
        f.ban_count = Math.max(0, +f.ban_count - 1);
        updateStats();
        renderForumList();
      }
    } catch (e) {
