$(document).ready(function () {
  ...
  async function fetchUsers(query) {
    try {
      const res = await fetch(
        `${BASE_URL}/api/php/users.php?action=search_users&q=${encodeURIComponent(query)}`,
      );
  ...
  function renderNavSearchResults(users) {
    ...
    users.forEach((user) => {
      ...
      let usernameHtml = user.full_name
        ? `<span class="fw-normal" style="color:#6c8a7a; font-size: 0.85rem; word-break: break-all;">@${user.username}</span>`
        : "";
      ...
      html += `
        <div class="rcw-user-card position-relative w-100" 
          ...
          <div class="row align-items-center px-3 py-3 m-0 g-3">
            ...
            <div class="col min-w-0">
              ...
              <small style="color: #6c8a7a; font-size: 0.78rem; word-break: break-word;">
                <i class="fa-solid ${locationIconClass} me-1" style="color: ${locationColor};"></i>${locationText}
              </small>
            </div>

            <!-- Action -->
            <div class="col-12 col-sm-auto text-start text-sm-end mt-2 mt-sm-0 position-relative" style="z-index: 2;">
              ${actionHtml}
            </div>
  ...
  function bindNavFriendActions() {
    $(".rcw-add-friend-btn")
      ...
      .on("click", async function (e) {
        ...
        try {
          const res = await fetch(
            `${BASE_URL}/api/php/users.php?action=friend_request`,
            {
              method: "POST",
              headers: {
                "X-CSRF-Token":
          ...
          if (data.success) {
            btn.replaceWith(
              `<span class="badge bg-secondary p-2 px-3 fw-bold rounded-0 mx-auto"><i class="fa-solid fa-clock me-1"></i> Pendiente</span>`,
            );
          } else {
    ...
    $(".rcw-accept-friend-btn")
      ...
      .on("click", async function (e) {
        ...
        try {
          const res = await fetch(
            `${BASE_URL}/api/php/users.php?action=accept_friend`,
            {
              method: "POST",
              headers: {
                "X-CSRF-Token":
          ...
          if (data.success) {
            btn.replaceWith(
              `<span class="badge bg-success p-2 px-3 fw-bold rounded-0 mx-auto"><i class="fa-solid fa-user-check me-1"></i> Amigos</span>`,
            );
          } else {
