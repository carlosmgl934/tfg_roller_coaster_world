...
async function loadDashboard() {
  try {
    const res = await fetch(`${BASE_URL}/api/php/index.php?action=dashboard`, {
      credentials: "include",
    });
    ...
    try {
      const crRes = await fetch(
        BASE_URL + "/api/php/admin/admin_carousel.php?action=get",
        { credentials: "include" },
      );
      ...
      if (crData.success) {
        crData.slides.forEach(function (s) {
          if (
            s.image_url &&
            typeof window.updateSingleCarouselSlide === "function"
          ) {
            ...
            if (slide)
              slide.style.backgroundImage = "url('" + s.image_url + "')";
          }
        });
      }
    } catch (_) {
...
function renderNews(news) {
  ...
  grid.innerHTML =
    ...
    `<div class="home-news-small-col">${small.map((n) => renderCard(n, false)).join("")}</div>`;
}
...
function openNewsModal(newsDataString) {
  ...
  try {
    ...
    document.getElementById("news-modal-tag").textContent =
      item.tag || "Novedad";
    document.getElementById("news-modal-date").textContent = timeAgo(
      item.created_at,
    );

    const imgEl = document.getElementById("news-modal-img");
    if (item.image_url) {
      imgEl.src = item.image_url.startsWith("http")
        ? item.image_url
        : BASE_URL + item.image_url;
    ...
    if (item.external_link && item.external_link.trim() !== "") {
      ...
      btn.innerHTML =
        'Ver más <i class="fa-solid fa-arrow-up-right-from-square ms-1"></i>';
...
function renderFeatures(totalCoasters) {
  ...
  const count = totalCoasters
    ? totalCoasters.toLocaleString("es-ES")
    : "10.000";
  ...
  const features = [
    {
      ...
      desc: `Más de ${count} coasters indexados de todo el mundo con datos técnicos completos.`,
    },
    {
      icon: "fa-list-ol",
      title: "Tops personalizados",
      desc: "Crea y ordena tu ranking personal. Compáralo con el de la comunidad.",
    },
    {
      icon: "fa-suitcase-rolling",
      title: "Gestor de viajes",
      desc: "Planifica tus rutas por parques, calcula distancias y gestiona tus trips.",
    },
    {
      icon: "fa-camera",
      title: "Galería de fotos",
      desc: "Sube fotos de tus visitas y descubre las de otros usuarios.",
    },
    {
      icon: "fa-comments",
      title: "Foros especializados",
      desc: "Debate sobre coasters, parques, noticias y novedades del sector.",
    },
    {
      icon: "fa-trophy",
      title: "Ranking global",
      desc: "Compite con otros enthusiasts y escala en el ranking de credits.",
    },
  ];
