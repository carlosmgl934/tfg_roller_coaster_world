...
(function () {
  ...
  const CURRENT_UID = window.CURRENT_USER
    ? parseInt(window.CURRENT_USER)
    : null;
  ...
  async function loadForum() {
    try {
      const res = await fetch(
        `${BASE}/api/php/forums.php?action=get_forum&forum_id=${FORUM_ID}`,
      );
      ...
      if (forum.privacy === "private" && role === "reader" && !IS_ADMIN) {
        disableInput(
          "Este foro es privado. Solo los colaboradores pueden escribir.",
        );
      }
      ...
      const openInfoModal = () => {
        ...
        el("forumInfoModalMembers").textContent =
          forum.member_count !== undefined ? forum.member_count : "—";

        if (forum.created_at) {
          const d = new Date(forum.created_at);
          el("forumInfoModalCreated").textContent = d.toLocaleDateString(
            "es-ES",
        ...
        if (forum.author_name) {
          el("forumInfoModalAuthor").innerHTML =
            `Creado por <a href="${BASE}/web/views/public/users/user_profile.php?id=${forum.author_id}" class="text-success text-decoration-none" style="transition: opacity 0.2s;" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">${esc(forum.author_name)}</a>`;
        } else {
          el("forumInfoModalAuthor").innerHTML = "";
        }

        const isPrivate = forum.privacy === "private";
        el("forumInfoModalPrivacyIcon").className = isPrivate
          ? "fa-solid fa-lock me-1"
          : "fa-solid fa-earth-europe me-1";
        el("forumInfoModalPrivacyText").textContent = isPrivate
          ? "Foro privado"
          : "Foro público";
        ...
        if (forum.collaborators_json) {
          try {
            ...
            console.warn("Error parsing collabs modal", e);
          }
        }
    ...
    try {
      ...
      if (initial) {
        ...
        msgs.forEach((m) => {
          ...
          const noMsgs = el("forum-messages-list").querySelector(
            ".forum-no-msgs",
          );
          if (noMsgs) noMsgs.remove();
          el("forum-messages-list").insertAdjacentHTML(
            "beforeend",
            renderMessage(m),
          );
        });
  ...
  function renderMessage(m) {
    ...
    const isOwnerMsg =
      forumRole === "owner" || forumRole === "collaborator" || IS_ADMIN;
    ...
    const contentHtml = m.content
      ? `<div class="msg-text">${linkify(esc(m.content))}</div>`
      : "";
  ...
  function bindMessageActions() {
    ...
    document.querySelectorAll(".hide-btn").forEach((btn) => {
      ...
      btn.addEventListener("click", async () => {
        ...
        const res = await fetch(
          `${BASE}/api/php/forums.php?action=hide_message`,
          {
            headers: {
              "X-CSRF-Token":
                document
        ...
        if (data.success) {
          ...
          if (wrap) {
            ...
            if (bubble) {
              if (hidden === 1) {
                if (!bubble.querySelector(".msg-hidden-banner")) {
                  ...
                  banner.innerHTML =
                    '<i class="fa-solid fa-eye-slash me-1"></i>Mensaje ocultado por el moderador';
                  const anchor =
                    bubble.querySelector(".msg-author") ||
                    bubble.querySelector(".msg-reply-quote") ||
                    bubble.firstChild;
                  bubble.insertBefore(
                    banner,
                    anchor?.nextSibling ?? bubble.firstChild,
                  );
                }
              } else {
            ...
            btn.querySelector("i").className =
              hidden === 1 ? "fa-solid fa-eye" : "fa-solid fa-eye-slash";
            btn.setAttribute(
              "data-tooltip",
              hidden === 1 ? "Mostrar mensaje" : "Ocultar mensaje",
            );
          }
    ...
    if (confirmBtn && !confirmBtn.dataset.bound) {
      ...
      confirmBtn.addEventListener("click", async () => {
        ...
        const res = await fetch(
          `${BASE}/api/php/forums.php?action=delete_message`,
          {
            headers: {
              "X-CSRF-Token":
                document
    ...
    if (confirmCollabBtn && !confirmCollabBtn.dataset.bound) {
      ...
      confirmCollabBtn.addEventListener("click", async () => {
        ...
        const res = await fetch(
          `${BASE}/api/php/forums.php?action=remove_collaborator`,
          {
            headers: {
              "X-CSRF-Token":
                document
        ...
        if (data.success) {
          showToast(
            `"${pendingRemoveCollabName}" eliminado de colaboradores`,
            "success",
          );
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (pendingFile) {
        ...
        if (!uploadResult.ok) {
          showToast(
            "Error al subir el archivo: " + uploadResult.error,
            "error",
          );
      ...
      const res = await fetch(
        `${BASE}/api/php/forums.php?action=send_message`,
        {
          headers: {
            "X-CSRF-Token":
              document
      ...
      if (data.success) {
        ...
        if (
          res.status === 429 ||
          (data.error && data.error.includes("esperar"))
        ) {
  ...
  async function uploadToSupabase(file) {
    try {
      ...
      const safeFilename = (fileToUpload.name || "img.webp").replace(
        /[^a-zA-Z0-9.-]/g,
        "_",
      );
  ...
  async function openModPanel() {
    ...
    if (inviteWrapper) {
      inviteWrapper.style.setProperty(
        "display",
        canManage ? "flex" : "none",
        "important",
      );
    ...
    const showParticipants =
      forumPrivacy === "public" && (canManage || canCollaborate);
    if (participantsSection)
      participantsSection.parentElement &&
        (participantsSection.style.display = showParticipants ? "" : "none");
    if (participantsTitle)
      participantsTitle.style.display = showParticipants ? "" : "none";
  ...
  async function loadCollaboratorsList(canInvite) {
    ...
    try {
      const res = await fetch(
        `${BASE}/api/php/forums.php?action=get_collaborators&forum_id=${FORUM_ID}`,
      );
      const data = await res.json();
      if (!data.success) {
        container.innerHTML =
          '<p class="text-danger small">Error al cargar</p>';
        return;
      }
      ...
      if (!collaborators || !collaborators.length) {
        container.innerHTML =
          '<p class="text-muted small">No hay colaboradores en este foro.</p>';
        return;
      }
  ...
  async function loadParticipants() {
    ...
    try {
      const res = await fetch(
        `${BASE}/api/php/forums.php?action=get_participants&forum_id=${FORUM_ID}`,
      );
      ...
      try {
        ...
        console.error(
          "[participants] Respuesta no-JSON:",
      ...
      if (!participants.length) {
        container.innerHTML =
          '<p class="text-muted small">Aún no hay participantes.</p>';
        return;
      }
      ...
      container.innerHTML = participants
        .map((p) => {
          ...
          const isCollab = currentCollaboratorsIds.includes(
            parseInt(p.user_id),
          );
          const showBan = canBan && !isSelf && !isAuthor;
          const msgLabel =
            parseInt(p.message_count) === 1 ? "mensaje" : "mensajes";
          const roleLabel =
            isCollab && !isAuthor
              ? '<span class="text-success fw-medium me-1">Colaborador &bull;</span>'
              : "";
  ...
  async function loadFriendsForInvite() {
    ...
    try {
      ...
      const availableFriends = data.friends.filter(
        (f) => !currentCollaboratorsIds.includes(parseInt(f.id)),
      );

      if (availableFriends.length === 0) {
        selectEl.innerHTML =
          '<option value="">No hay amigos disponibles para invitar</option>';
        return;
      }
        ...
        btn.addEventListener("click", async () => {
          ...
          try {
            const iRes = await fetch(
              `${BASE}/api/php/forums.php?action=invite_collaborator`,
              {
                headers: {
                  "X-CSRF-Token":
                    document
  ...
  async function loadBannedList() {
    ...
    try {
      const res = await fetch(
        `${BASE}/api/php/forums.php?action=get_banned&forum_id=${FORUM_ID}`,
      );
      const data = await res.json();
      if (!data.success) {
        container.innerHTML =
      ...
      if (!banned.length) {
        container.innerHTML =
          '<p class="text-muted small">No hay usuarios baneados.</p>';
        return;
      }
      ...
      document.querySelectorAll(".unban-btn").forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          const res = await fetch(
            `${BASE}/api/php/forums.php?action=unban_user`,
            {
              headers: {
                "X-CSRF-Token":
                  document
  ...
  function startPolling() {
    ...
    document.addEventListener("visibilitychange", () => {
      if (document.hidden) {
        ...
        syncForumState().then(() => {
          if (!isBannedSelf) loadMessages(false);
        });
        pollTimer = setInterval(async () => {
          await syncForumState();
  ...
  async function syncForumState() {
    try {
      const res = await fetch(
        `${BASE}/api/php/forums.php?action=get_forum_state&forum_id=${FORUM_ID}`,
      );
      const data = await res.json();
      if (!data.success) return;
      ...
      if (
        list &&
        !list.querySelector("[data-msg-id]") &&
        !list.querySelector(".forum-no-msgs")
      ) {
        list.innerHTML = `
          <div class="forum-no-msgs">
            <i class="fa-regular fa-comment-dots fa-3x mb-3 opacity-25"></i>
      ...
      document.querySelectorAll("[data-msg-id]").forEach((wrap) => {
        ...
        if (hidden && !isHiddenNow) {
          ...
          if (bubble && !bubble.querySelector(".msg-hidden-banner")) {
            ...
            bubble.insertBefore(
              banner,
              firstChild?.nextSibling ?? bubble.firstChild,
            );
          }
