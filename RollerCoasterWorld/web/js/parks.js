$(document).ready(function () {
  ...
  if (document.getElementById("park-name")) {
    ...
    async function loadParkPhotos(id) {
      try {
        const res = await fetch(`${apiBase}?action=get_photos&park_id=${id}`);
        const data = await res.json();
        const grid = $("#photos-grid");
        const countBadge = $("#photos-count");

        if (data.success && data.data && data.data.length > 0) {
          grid.empty();
          countBadge.text(data.data.length);
          data.data.forEach((photo) => {
            grid.append(`
              <div class="col-6 col-md-4 col-lg-3">
                <div class="photo-card position-relative overflow-hidden shadow-sm h-100" 
                     style="cursor:pointer; aspect-ratio:1/1;"
                     onclick="openParkLightbox('${photo.photo_url}', '${photo.username}', '${photo.profile_image}', '${photo.caption}')">
                  <img src="${photo.photo_url}" class="w-100 h-100 object-fit-cover transition-all" alt="Foto parque">
                  <div class="photo-overlay position-absolute bottom-0 start-0 w-100 p-2 text-white small" 
                       style="background: linear-gradient(transparent, rgba(0,0,0,0.8)); opacity:0; transition: opacity 0.3s ease;">
                    <i class="fa-solid fa-user me-1"></i> ${photo.username}
                  </div>
                </div>
              </div>
            `);
          });
          // Añadir hover effect vía CSS dinámico si no existe
          if (!document.getElementById("photo-hover-css")) {
            $("head").append(`
              <style id="photo-hover-css">
    ...
    window.openParkLightbox = function (url, user, avatar, caption) {
      const modal = new bootstrap.Modal(document.getElementById("ig-lightbox-modal"));
      $("#ig-modal-img").attr("src", url);
      $("#ig-modal-username").text(user);
      $("#ig-modal-caption-user").text(user);
      $("#ig-modal-caption").text(caption || "");
      $("#ig-modal-avatar").attr("src", avatar || "https://ui-avatars.com/api/?name=" + user);
      modal.show();
    };
    ...
    uploadBtn?.addEventListener("click", async function () {
      ...
      uploadBtn.innerHTML = 'Subiendo... <i class="fa-solid fa-spinner fa-spin ms-2"></i>';

      try {
        cropper.getCroppedCanvas({ width: 1080, height: 1080 }).toBlob(async function (blob) {
          const formData = new FormData();
          formData.append("file", blob, "park_photo.jpg");
          formData.append("bucket", "parks");
          formData.append("path", parkId);

          // 1. Subir al storage (upload.php)
          const upRes = await fetch(`${window.BASE_URL}/api/php/upload.php`, {
            method: "POST",
            body: formData,
          });
          ...
          saveForm.append("park_id", parkId);
          saveForm.append("photo_url", upData.url);
          saveForm.append("caption", document.getElementById("photo-caption").value);

          const saveRes = await fetch(`${apiBase}?action=add_photo`, {
            method: "POST",
            body: saveForm,
          });
          ...
          if (saveData.success) {
            bootstrap.Modal.getInstance(document.getElementById("upload-photo-modal")).hide();
            loadParkPhotos(parkId);
            showNotify("¡Foto subida!", "La foto se ha guardado correctamente.");
          } else {
            showNotify("Error", saveData.error, true);
          }
        }, "image/jpeg", 0.8);
      } catch (err) {
    ...
    function showNotify(title, msg, isError = false) {
      const modal = new bootstrap.Modal(document.getElementById("notify-modal"));
      $("#notify-modal-title").text(title).toggleClass("text-danger", isError).toggleClass("text-success", !isError);
      $("#notify-modal-body").text(msg);
      modal.show();
    }
  }
